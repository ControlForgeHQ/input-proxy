...
static char *render_rule(const struct input_proxy_device_rule_identity *identity,
    bool permission, bool ignore)
{
    ...
    struct input_proxy_installation_application_failure *failure)
{
    ...
    if (resolution->source_permission_action || resolution->libinput_ignore_action) {
    ...
    if (content != NULL && !publish_rule(directory, final_rule, content,
