...
assert_cli(success "Usage:.*input-proxy install \\[OPTIONS\\].*--source PATH.*--name INSTANCE_NAME.*--activity-timeout-ms MS.*--detection-throttle-ms MS.*--running-motion-activity on\\|off.*--paused-motion-activity on\\|off.*--start-paused on\\|off.*--use-preferred-run-source yes\\|no.*--add-libinput-ignore-rule yes\\|no" install --help)
assert_cli_streams(success "Plan installation" "^$" install --help)
assert_cli(success "Remove one persistent input-proxy Installed Instance.*Usage:.*input-proxy uninstall \\[INSTANCE_NAME\\].*interactive selection.*--help" uninstall --help)
assert_cli_streams(success "Remove one persistent" "^$" uninstall --help)
...
assert_cli_blank_line(success install --help)
assert_cli_blank_line(success uninstall --help)
assert_cli_blank_line(failure install)
assert_cli_blank_line(failure uninstall)
