...
and continue normal input forwarding. It then retries the complete initialization
sequence at a fixed five-second monotonic cadence. Recovery rebuilds and exports
the complete object before requesting the same well-known name without queueing.
It exposes the session's current pause and source-availability state, while both
activity properties restart at false. Failed recovery attempts remain silent in
standard logging and leave normal proxy operation unaffected.
